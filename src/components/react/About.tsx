...
export default function About() {
	...
	let description;
	if (props.number % 2 == 0) {
		description = <Accent>even</Accent>;
	} else {
		description = <i>odd</i>;
	}
		...
		<div className="grey-card">
			...
			/>
			{
				...
				<>
					...
					<p>JSX - almost all React code is written in JSX. JSX is a syntax extension of JS that allows us to write HTML-like syntax in JS.</p>
					...
					<SyntaxHighlighter language="javascript">{jsx}</SyntaxHighlighter>
					...
					<SyntaxHighlighter language="javascript">{statements}</SyntaxHighlighter>
					...
					<p>Stateful component. The state has a default state. To update the state React expects you use built-in <Accent>setState()</Accent> method (asynchronous).</p>
					...
					<p><Accent>State Hooks</Accent> - state lets a component “remember” information like user input.</p>
					<ul>
					...
					<p><Accent>Context Hooks</Accent> - Context lets a component receive information from distant parents without passing it as props.</p>
					<ul>
						<li>useContext reads and subscribes to a context.</li>
					</ul>
					<p><Accent>Ref Hooks</Accent> - Refs let a component hold some information that isn’t used for rendering, like a DOM node or a timeout ID. Unlike with state, updating a ref does not re-render your component. Refs are an “escape hatch” from the React paradigm. They are useful when you need to work with non-React systems, such as the built-in browser APIs.</p>

					<p><Accent>Effect Hooks</Accent> - Effects let a component connect to and synchronize with external systems. This includes dealing with network, browser DOM, animations, widgets written using a different UI library, and other non-React code.</p>
					<ul>
						<li>useEffect connects a component to an external system.</li>
					</ul>

					<p><Accent>Performance Hooks</Accent> - A common way to optimize re-rendering performance is to skip unnecessary work. For example, you can tell React to reuse a cached calculation or to skip a re-render if the data has not changed since the previous render.</p>
					<ul>
					...
					<SyntaxHighlighter language="javascript">{useMemoCode}</SyntaxHighlighter>
				</>
			}
		</div>
	);
