		...
		<div className="grey-card">
			...
			/>
			{
				...
				<>
					...
					<p>Note that when invoked with multiple numbers, the <Accent>Date()</Accent> constructor interprets them using whatever time zone the local computer is set to. If you want to specify a date and time in UTC (Universal Coordinated Time, aka GMT), then you can use the <Accent>Date.UTC()</Accent>.</p>
					...
					<SyntaxHighlighter language="javascript">{runtime}</SyntaxHighlighter>
					...
					<ul>
						...
						<li>Date setting methods work <Accent>correctly even when they overflow</Accent>. When we add three months to the current month, we can end up with a value greater than 11 (which represents December). The setMonth() handles this by incrementing the year as needed. Similarly, when we set the day of the month to a value larger than the number of days in the month, the month gets incremented appropriately.</li>
					</ul>
					<SyntaxHighlighter language="javascript">{arithmetic}</SyntaxHighlighter>
					<p className="card-section">Formatting and Parsing Date String</p>
					<SyntaxHighlighter language="javascript">{formatting}</SyntaxHighlighter>
				</>
			}
		</div>
	);
